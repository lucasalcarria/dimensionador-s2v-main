...
## Corpus Check
- 24 files · ~37,613 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 341 nodes · 532 edges · 20 communities (10 shown, 10 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 21 edges (avg confidence: 0.84)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1adf8d4c`
...
5. `Entradas` - 16 edges
6. `_textos()` - 13 edges
7. `gerar_proposta()` - 13 edges
8. `carregar_config()` - 12 edges
...
Cohesion: 0.08
Nodes (23): Dados do usuário: nunca versionar/sobrescrever, Retorno de 25 anos (compat_planilha, bug do COUNTA corrigido), config.json (parâmetros de negócio), Usar no celular via QR code / mesma rede Wi-Fi, Parâmetros editáveis em config.json, Dimensionador S2V — proposta solar fotovoltaica (LEIA-ME), Estrutura de pastas do projeto, Executável único (.exe) via build_windows.bat (+15 more)
