...
## Corpus Check
- 25 files · ~40,194 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 393 nodes · 645 edges · 26 communities (16 shown, 10 thin omitted)
...
Cohesion: 0.06
Nodes (47): Dimensionador S2V (project), Regra número 1: engine.py réplica validada da planilha, Caso de validação NEUZA ZAMFERRARI (UC 16285387), % noturno: GERADORA usa valor informado, BENEFICIÁRIA sempre 100%, _exemplo_enunciado(), Reproduz o caso descrito: A=600 (geradora), B=400 (beneficiária),     geração ~1, relatorio_conferencia(), dict (+39 more)
...
Cohesion: 0.12
Nodes (15): api_drive_desconectar(), _exige_login(), icones(), login(), manifest(), oauth2_callback(), Descreve o app para o celular (nome, ícones, cor) — vira ícone na tela., Service worker. Servido da raiz (/sw.js) de propósito: só assim ele     controla (+7 more)
...
Cohesion: 0.20
Nodes (11): api_calcular(), api_concessionarias(), api_conferencia(), api_copel_verificar(), index(), _montar_entradas(), Devolve o detalhamento passo a passo do retorno financeiro., Concessionárias: tarifas locais por subgrupo + alíquotas + subgrupos. (+3 more)

### Community 24 - "_f"
Cohesion: 0.18
Nodes (11): api_atualizar_tarifa(), api_irradiacao(), api_salvar_config(), _f(), Converte número vindo da tela (aceita vírgula) — vazio vira `padrao`., Grava TE/TUSD de uma concessionária+subgrupo na tabela local.      Aceita valore, Grava as pré-definições editadas, preservando o resto do config.json     (coment, caminho_config() (+3 more)
